...
namespace HenryDev
{
    ...
    public static class CommonExtension
    {
        ...
        public static void Follow(this Transform current, Vector3 to, float speed = 1)
        {
            var currentPosition = current.position;
            currentPosition = Vector3.MoveTowards(currentPosition, to, speed * Time.deltaTime);
            current.position = currentPosition;
        }
        ...
        public static void Follow(this Rigidbody2D current, Vector3 to, float speed = 1)
        {
            Vector3 direction = to - current.transform.position;
            current.velocity = direction.normalized * speed;
        }
        public static void Follow(this Rigidbody current, Transform to = null, float speed = 1)
        {
            if (to == null)
                return;
            Vector3 direction = to.position - current.transform.position;
        ...
        public static void Follow(this Rigidbody current, Vector3 to, float speed = 1)
        {
            Vector3 direction = to - current.transform.position;
            current.velocity = direction.normalized * speed;
        }
        ...
        public static void FollowAndRemainDistance(this Transform current, Vector3 to, float speed = 1, float recoverySpeed = 1, float stoppingDistance = 0f, float stoppingDistanceBuffer = 0f)
        {
            float backSpeed = -recoverySpeed;
            var currentPosition = current.position;
            if (current.transform.IsAtRange(to, stoppingDistance, stoppingDistanceBuffer))
                currentPosition = Vector3.MoveTowards(currentPosition, to, 0 * Time.deltaTime);
            else if (current.transform.IsNear(to, stoppingDistance, stoppingDistanceBuffer))
                currentPosition = Vector3.MoveTowards(currentPosition, to, backSpeed * Time.deltaTime);
            else
                currentPosition = Vector3.MoveTowards(currentPosition, to, speed * Time.deltaTime);
        ...
        public static void RotateToward(this Transform current, Vector3 targetDirection, float rotateSpeed,  [DefaultValue("Vector3.up")] Vector3 upAxis, eAxis lockedAxis = eAxis.Origin)
        {
            var targetRotation = Quaternion.LookRotation(targetDirection, upAxis);
            Quaternion nextRotation = Quaternion.Slerp(current.rotation, targetRotation, rotateSpeed * Time.deltaTime);
            current.rotation = nextRotation;
        ...
        public static Vector2 RandomPositionOnCircle(this Vector2 pos, float radius)
        {
            float angle = Random.Range(0, 2 * Mathf.PI);
            float randomX = pos.x + radius * Mathf.Cos(angle);
            float randomY = pos.y + radius * Mathf.Sin(angle);
            return new Vector2(randomX, randomY);
        }
        ...
        public static bool IsAtRange(this Transform current, Vector3 target, float distance, float buffer = 0.1f)
        {
            float squareValue = Mathf.Pow(target.x - current.position.x, 2) + Mathf.Pow(target.y - current.position.y, 2) + Mathf.Pow(target.z - current.position.z, 2);
            return distance * distance - buffer * buffer <= squareValue && squareValue <= distance * distance + buffer * buffer;
        }
        ...
        public static Vector3 GetDirectionTo(this Vector3 current, Vector3 target)
        {
        ...
        public static Vector3 GetNormalizedDirectionTo(this Vector3 current, Vector3 target)
        {
            return current.GetDirectionTo(target).normalized;
        }
